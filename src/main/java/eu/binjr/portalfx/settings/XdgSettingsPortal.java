...
import eu.binjr.portalfx.xdg.AbstractDesktopPortal;
 ...
 */
public class XdgSettingsPortal extends AbstractDesktopPortal implements SettingsPortal {
    ...
    public XdgSettingsPortal() throws DBusException {
        ...
        XdgSettingsInterface settings = this.getPortalObject(XdgSettingsInterface.class);
        var appearance = settings.ReadAll(new String[]{FREEDESKTOP_APPEARANCE}).get(FREEDESKTOP_APPEARANCE);
        if (appearance != null) {
            appearance.forEach(settingsMapper);
        }

        // Install signal handler to listen to org.freedesktop.portal.Settings::SettingChanged
        this.addSignalHandler(XdgSettingsInterface.SettingChanged.class, settings,
                signal -> {
